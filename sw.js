...
const CACHE = 'casa-v5';
const ASSETS = ['CASA.html', 'manifest.webmanifest', 'icon-192.png', 'icon-512.png', 'apple-touch-icon.png', 'favicon.png'];
...
self.addEventListener('fetch', e => {
  ...
  e.respondWith(
    fetch(req).then(res => {
      ...
      return res;
    }).catch(() => caches.match(req).then(r => r || caches.match('CASA.html')))
  );
});
